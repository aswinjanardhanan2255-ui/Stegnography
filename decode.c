...
d_Status read_and_validate_decode_args(char *argv[], DecodeInfo *decInfo) {
    ...
    if (argv[3] != NULL) 
    {
        ...
        {
            printf("INFO : Invalid output file extension\n");
            return d_failure;
        }
    } 
    else 
...
d_Status do_decoding(DecodeInfo *decInfo)
{
    if (open_files_decode(decInfo) == d_success)
    {
        ...
        char magic[3];
        int extn_size, file_size;
        char file_extn[10];
        char secret_data[1000];

        if (decode_magic_string(magic, strlen(MAGIC_STRING), decInfo) == d_success)
        {
            printf("\n##Decoding starts from here##\n");
            printf("\nDecoded magic string success\n");
